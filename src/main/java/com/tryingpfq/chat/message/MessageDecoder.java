...
public class MessageDecoder extends ByteToMessageDecoder {
	...
	public IMMessage decode(String msg){
		...
		try{
			...
			if(msg.startsWith("[" + MessageType.LOGIN.getName() + "]")){
				return new IMMessage(heards[0],time,nickName);
			}else if(msg.startsWith("[" + MessageType.CHAT.getName() + "]")){
				return new IMMessage(heards[0],time,nickName,content);
			}else if(msg.startsWith("[" + MessageType.FLOWER.getName() + "]")){
				return new IMMessage(heards[0],time,nickName);
			}else{
				return null;
			}
