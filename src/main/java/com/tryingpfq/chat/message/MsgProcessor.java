...
public class MsgProcessor {
	...
	public void logout(Channel client){
		...
		for (Channel channel : onlineUsers) {
			IMMessage request = new IMMessage(MessageType.SYSTEM.getName(), sysTime(), onlineUsers.size(), getNickName(client) + "离开");
	...
	public void sendMsg(Channel client,String msg){
		...
		if(request.getCmd().equals(MessageType.LOGIN.getName())){
			...
			for (Channel channel : onlineUsers) {
				if(channel != client){
					request = new IMMessage(MessageType.SYSTEM.getName(), sysTime(), onlineUsers.size(), getNickName(client) + "加入");
				}else{
					request = new IMMessage(MessageType.SYSTEM.getName(), sysTime(), onlineUsers.size(), "已与服务器建立连接！");
				}
				String content = encoder.encode(request);
				channel.writeAndFlush(new TextWebSocketFrame(content));
			}
		}else if(request.getCmd().equals(MessageType.CHAT.getName())){
			for (Channel channel : onlineUsers) {
				if (channel == client) {
		...
		}else if(request.getCmd().equals(MessageType.FLOWER.getName())){
			...
			if(null != attrs){
				...
				if(sub < 1000 * secends){
					request.setSender("you");
					request.setCmd(MessageType.SYSTEM.getName());
